...
class MainActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val storiesTitle = arrayOf("The bee and the grasshopper ","Chicken Little","The hungry cat","The ugly duckling")
        val sound = arrayOf(R.raw.bee,R.raw.chicken,R.raw.cat,R.raw.duck)
...
""".trimIndent()
        ...
        val adapter = ArrayAdapter(this,android.R.layout.simple_list_item_1,storiesTitle)
        val list : ListView = findViewById(R.id.lv)
        list.adapter = adapter

        val setting_btn : Button = findViewById(R.id.setting_btn)

        setting_btn.setOnClickListener {
            val k = Intent(this,SettingActivity::class.java)
            startActivity(k)
        }
        ...
        list.setOnItemClickListener { adapterView, view, i, l ->
            ...
            val x = Intent(this,StoryDetails::class.java)
            x.putExtra("sound",sound[i])
            x.putExtra("picture",pictures[i])
            x.putExtra("title",storiesTitle[i])
            x.putExtra("story",stories[i])
            startActivity(x)
